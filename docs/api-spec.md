...
모든 응답은 성공/실패 여부와 무관하게 아래 4개 필드로 감싸서 내려간다.
프론트가 응답 하나를 두 가지 형태로 분기해서 파싱하지 않도록 하기 위함이다.

| 필드 | 타입 | 설명 |
| --- | --- | --- |
| `isSuccess` | boolean | 성공 여부 |
| `code` | string | HTTP 상태 코드보다 세분화된 자체 코드 (아래 규칙 참고) |
| `message` | string | code에 대한 사람이 읽을 수 있는 설명 |
| `result` | object | 실제 데이터. 값이 없으면 필드 자체가 생략된다 |
...
**유효성 검증 실패 응답 예시** (`@Valid` 실패, 필드별 사유를 result에 담는다)
...
`[도메인][HTTP 상태 앞자리]00[일련번호]` 형태로 관리한다.
HTTP 상태 코드만으로는 "400인 건 알겠는데 정확히 뭐가 잘못됐는지"를 알 수 없어서,
어느 도메인의 어떤 상황인지를 code 자체에 담는다.

| 코드 | HTTP | 설명 |
| --- | --- | --- |
| `COMMON200` / `COMMON201` | 200 / 201 | 일반 성공 |
| `COMMON400` | 400 | 잘못된 요청 (유효성 검증 실패 포함) |
| `COMMON401` / `COMMON403` | 401 / 403 | 인증 필요 / 권한 없음 |
| `COMMON500` | 500 | 서버 에러 |
| `USER4001` | 404 | 해당 회원을 찾을 수 없음 |
| `USER4002` | 409 | 이미 사용 중인 이메일 |
| `FRIEND4002` | 409 | 이미 친구이거나 요청이 존재함 |
| `FRIEND4003` | 400 | 자기 자신에게 친구 요청 |
| `ROOM4001` | 404 | 채팅방 없음 |
| `ROOM4002` | 403 | 해당 방의 참여자가 아님 |
| `MESSAGE4003` | 400 | 페이지 크기가 1~100 범위 밖 |
| `USER4005` | 401 | 이메일 또는 비밀번호 불일치 |
| `FRIEND4004` | 403 | 본인이 받은 친구 요청이 아님 |
| `FRIEND4005` | 409 | 이미 수락된 친구 요청 |
| `ROOM4004` | 400 | 1:1 방에 상대방을 2명 이상 지정 |

### 인증

`POST /api/auth/login`으로 받은 액세스 토큰을 이후 요청의
`Authorization: Bearer <token>` 헤더에 실어 보낸다.
로그아웃한 토큰은 서버 블랙리스트에 등록되어 만료 전이라도 거부된다.

> 아래 각 API의 **응답 body**는 위 `result` 안에 들어가는 내용만 표기한다.
...
> **왜 커서 기반인가**
> 채팅처럼 계속 새 메시지가 쌓이는 목록에서 offset(`page`/`size`) 방식은
> (1) 조회 도중 새 메시지가 들어오면 페이지 경계가 밀려 같은 메시지가 중복되거나 누락되고,
> (2) 뒤쪽 페이지로 갈수록 `OFFSET N`을 건너뛰는 비용이 커진다.
> 그래서 마지막으로 읽은 `message_id`를 커서로 넘겨 그 지점부터 읽는다.
> `hasNext`는 `size + 1`건을 읽어 초과분이 있는지로 판단하고, 응답에는 `size`건만 담는다.
>
> 목록 성격이 다른 API(정렬·건너뛰기가 필요한 목록)를 위해
> offset 기반 `PageResponse<T>`도 `apiPayload/paging`에 함께 두었다.
