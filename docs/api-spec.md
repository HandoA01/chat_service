...
| 설명 | 특정 채팅방의 메시지 내역을 페이징으로 조회합니다. (최신순) |
| 인증 필요 | ✅ |
...
> **커서 기반 페이징에 대한 메모**
> 초안에서는 `cursor` 기반으로 설계했으나, 우선 Spring Data의 `Pageable`(offset 기반)로 구현했다.
> 채팅처럼 계속 새 메시지가 쌓이는 목록에서 offset 방식은
> (1) 조회 도중 새 메시지가 들어오면 페이지 경계가 밀려 같은 메시지가 중복/누락되고,
> (2) 뒤쪽 페이지로 갈수록 `OFFSET N`을 건너뛰는 비용이 커진다.
> 따라서 실서비스 단계에서는 `cursor`(마지막으로 읽은 message_id) 기반으로 전환할 예정이다.
